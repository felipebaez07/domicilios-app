...
/* ── WazeBtn: dos etapas origen → destino ─────────────── */
function WazeBtn({ pedido }) {
  const [etapa, setEtapa] = useState('origen');

  if (!pedido) return null;

  const tieneOrigen  = pedido.lat_origen  && pedido.lng_origen;
  const tieneDestino = pedido.lat_destino && pedido.lng_destino;
  const dirOrigen    = pedido.direccion_origen || '';
  const dirDestino   = pedido.direccion_entrega || pedido.direccion_destino || '';

  if (!tieneOrigen && !tieneDestino && !dirOrigen && !dirDestino) return null;

  function abrirWaze(lat, lng, dir) {
    function lanzar(fromLat, fromLng) {
      const fromPart = fromLat && fromLng ? `&from=${fromLat},${fromLng}` : '';
      let deepLink, webUrl;

      if (lat && lng) {
        // waze:// scheme — abre la app en Android/iOS directamente
        deepLink = `waze://?ll=${lat},${lng}&navigate=yes${fromPart}`;
        webUrl   = `https://waze.com/ul?ll=${lat},${lng}&navigate=yes${fromPart}&zoom=17`;
      } else if (dir) {
        const q = encodeURIComponent(dir + ', Ibagué, Tolima, Colombia');
        deepLink = `waze://?q=${q}&navigate=yes${fromPart}`;
        webUrl   = `https://waze.com/ul?q=${q}&navigate=yes${fromPart}`;
      } else return;

      // En móvil: intentar app scheme con window.location; si no responde en 2s, abrir web
      const start = Date.now();
      window.location.href = deepLink;
      setTimeout(() => {
        // Si la página sigue visible (no cambió a la app), abrir web
    ...
    if (navigator.geolocation) {
      const timer = setTimeout(() => lanzar(null, null), 3000);
      navigator.geolocation.getCurrentPosition(
        pos => { clearTimeout(timer); lanzar(pos.coords.latitude, pos.coords.longitude); },
        ()  => { clearTimeout(timer); lanzar(null, null); },
...
export default function DomiciliarioDashboard() {
  ...
  /* Ruta OSRM: mi posición → destino del pedido */
  useEffect(() => {
    if (!pedidoActivo || !myPos) { setRouteCoords([]); return; }
    const { lat_destino, lng_destino } = pedidoActivo;
    if (!lat_destino || !lng_destino) return;
    const [lat, lng] = Array.isArray(myPos) ? myPos : [myPos.lat, myPos.lng];
    fetch(`https://router.project-osrm.org/route/v1/driving/${lng},${lat};${lng_destino},${lat_destino}?overview=full&geometries=geojson`)
      .then(r => r.json())
      .then(data => {
        if (data.routes?.[0]) {
          setRouteCoords(data.routes[0].geometry.coordinates.map(([lo, la]) => [la, lo]));
        }
      })
      .catch(() => {});
  }, [pedidoActivo, myPos]);
  ...
  const gamStats = useGamification([...pedidoActivo ? [pedidoActivo] : [], ...pedidosHoy], user?.id);
