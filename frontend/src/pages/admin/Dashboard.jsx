    ...
    <DashboardLayout role="admin" pageTitle="Métricas">
      <div className="page-header">
        <div>
          ...
          <div className="page-subtitle">Vista global · {total} pedidos registrados</div>
        </div>
        <button className="btn btn-ghost" onClick={() => { setLoading(true); axios.get(`${API}/pedidos/todos`,{headers:{Authorization:`Bearer ${token}`}}).then(r=>setPedidos(r.data)).finally(()=>setLoading(false)); }}>🔄 Sync</button>
      </div>
      ...
      <div className="white-card">
        ...
        <div style={{ overflowX:'auto' }}>
          <table className="rv-table">
            ...
            <tbody>
              {loading ? <tr><td colSpan={6} style={{padding:'2rem',textAlign:'center',color:'#aaa'}}>⏳ Cargando...</td></tr>
                : paged.map(p => { const est=ESTADO[p.estado]||{label:p.estado,cls:'badge-neutral',emoji:'📌'}; return (
                  <tr key={p.id}>
