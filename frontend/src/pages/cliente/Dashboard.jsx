...
export default function ClienteDashboard() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const { data } = await axios.get(`${PEDIDOS_URL}/pedidos/mis-pedidos`, { headers:{ Authorization:`Bearer ${token}` } });
      setPedidos(data);
  ...
  const mapCenter    = domiPos || (tieneOrigen ? [selected.lat_origen,selected.lng_origen] : null) || [4.4389,-75.2322];

  // Si está en el picker de mapa, mostrar pantalla completa
  if (step === 'picker') {
    return (
  ...
  return (
    <DashboardLayout role="cliente" pageTitle="Mis pedidos">
      ...
      <div className="page-header">
        ...
        <button className="btn btn-ghost" onClick={() => setStep('picker')} style={{flexShrink:0}}>
          ➕ Nuevo pedido
        </button>
      </div>

      {/* Mobile: stack vertical / Desktop: split */}
      <div style={{ display:'grid', gridTemplateColumns:'clamp(220px,30%,280px) 1fr', height:'calc(100vh - 180px)', minHeight:400 }}>
          ...
          {loading
            ...
            : pedidos.map(p => (
              <button key={p.id} onClick={() => setSelected(p)} style={{
                ...
                <div style={{fontSize:12,fontWeight:700,color:'#fff',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{p.descripcion||'Pedido'}</div>
                <div style={{fontSize:10,color:'rgba(255,255,255,.6)',marginTop:2,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{p.direccion_entrega||p.direccion_destino}</div>
              </button>
            ))}
        </div>
        ...
        <div style={{display:'flex',flexDirection:'column',overflow:'hidden'}}>
          <div style={{flex:1,position:'relative',overflow:'hidden'}}>
            ...
            <div style={{width:'100%',height:'100%'}}>
              <AppMap center={mapCenter} zoom={14}>
                {ruta && ruta.length > 1 && <Polyline positions={ruta} color="#8b5cf6" weight={5} opacity={0.85}/>}
                {tieneOrigen  && <Marker position={[selected.lat_origen,selected.lng_origen]}   icon={origenIcon}><Popup>📦 Origen: {selected.direccion_origen}</Popup></Marker>}
                {tieneDestino && <Marker position={[selected.lat_destino,selected.lng_destino]} icon={destinoIcon}><Popup>🏠 Destino: {selected.direccion_entrega||selected.direccion_destino}</Popup></Marker>}
          ...
          {selected && selected.estado!=='cancelado' && (
            <div style={{padding:'.85rem 1.25rem',background:'rgba(255,255,255,.1)',borderTop:'1px solid rgba(255,255,255,.15)',flexShrink:0}}>
              <div style={{fontSize:10,color:'rgba(255,255,255,.6)',fontWeight:600,textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'.65rem'}}>Progreso</div>
                ...
                {PASOS.map((paso,i) => {
                  ...
                  return (
                    <div key={paso} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4,position:'relative'}}>
                      {i<PASOS.length-1 && <div style={{position:'absolute',top:11,left:'50%',width:'100%',height:3,background:done&&i<stepIdx?'rgba(255,255,255,.8)':'rgba(255,255,255,.2)',zIndex:0,borderRadius:2}}/>}
                      ...
                      <span style={{fontSize:9,color:done?'#fff':'rgba(255,255,255,.5)',textAlign:'center',fontWeight:cur?700:400}}>{EMOJIS[paso]} {LABELS[paso]}</span>
                    </div>
                  );
                })}
              </div>
      ...
      {/* Modal formulario después de elegir ubicaciones */}
          ...
          <div style={{padding:'1.25rem',overflow:'auto'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:6,marginBottom:'1rem'}}>
              <div style={{display:'flex',gap:10,padding:'8px 10px',background:'#f0faf5',borderRadius:10,border:'1px solid #86efac',alignItems:'flex-start'}}>
                ...
                <button onClick={() => setStep('picker')} style={{marginLeft:'auto',fontSize:10,color:'#aaa',background:'none',border:'none',cursor:'pointer'}}>✏️</button>
              </div>
